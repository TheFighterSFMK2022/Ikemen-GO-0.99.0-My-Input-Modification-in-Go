...
type KeyConfig struct{ Joy, dU, dD, dL, dR, kA, kB, kC, kX, kY, kZ, kS, kD, kW, kH, kT, kM int }
...
func (kc KeyConfig) h() bool { return JoystickState(kc.Joy, kc.kH) }
func (kc KeyConfig) t() bool { return JoystickState(kc.Joy, kc.kT) }
...
const (
	...
	IB_T
	IB_M
	IB_anybutton = IB_A | IB_B | IB_C | IB_X | IB_Y | IB_Z | IB_S | IB_D | IB_W | IB_H | IB_T
)
...
func (ib *InputBits) SetInput(in int) {
	if 0 <= in && in < len(sys.keyConfig) {
		*ib = InputBits(Btoi(sys.keyConfig[in].U() || sys.joystickConfig[in].U()) |
			...
			Btoi(sys.keyConfig[in].h() || sys.joystickConfig[in].h())<<13 |
			Btoi(sys.keyConfig[in].t() || sys.joystickConfig[in].t())<<14 |
			Btoi(sys.keyConfig[in].m() || sys.joystickConfig[in].m())<<15)
	}
}
func (ib InputBits) GetInput(cb *CommandBuffer, facing int32) {
	...
	cb.Input(B, ib&IB_PD != 0, F, ib&IB_PU != 0, ib&IB_A != 0, ib&IB_B != 0,
		...
		ib&IB_D != 0, ib&IB_W != 0, ib&IB_H != 0, ib&IB_T != 0, ib&IB_M != 0)
}

type CommandKeyRemap struct {
	a, b, c, x, y, z, s, d, w, h, t, m, na, nb, nc, nx, ny, nz, ns, nd, nw, nh, nt, nm CommandKey
}

func NewCommandKeyRemap() *CommandKeyRemap {
	return &CommandKeyRemap{CK_a, CK_b, CK_c, CK_x, CK_y, CK_z, CK_s, CK_d, CK_w, CK_m, CK_h, CK_t,
		CK_na, CK_nb, CK_nc, CK_nx, CK_ny, CK_nz, CK_ns, CK_nd, CK_nw, CK_nh, CK_nt, CK_nm}
}

type CommandBuffer struct {
	Bb, Db, Fb, Ub                                 int32
	ab, bb, cb, xb, yb, zb, sb, db, wb, hb, tb, mb int32
	B, D, F, U                                     int8
	a, b, c, x, y, z, s, d, w, h, t, m             int8
}
...
func (__ *CommandBuffer) Reset() {
	*__ = CommandBuffer{B: -1, D: -1, F: -1, U: -1,
		a: -1, b: -1, c: -1, x: -1, y: -1, z: -1, s: -1, d: -1, w: -1, h: -1, t: -1, m: -1}
}
func (__ *CommandBuffer) Input(B, D, F, U, a, b, c, x, y, z, s, d, w, h, t, m bool) {
	if (B && !F) != (__.B > 0) {
		__.Bb = 0
		__.B *= -1
	}
...
func (__ *CommandBuffer) InputBits(ib InputBits, f int32) {
	...
	__.Input(B, ib&IB_PD != 0, F, ib&IB_PU != 0, ib&IB_A != 0, ib&IB_B != 0,
		ib&IB_C != 0, ib&IB_X != 0, ib&IB_Y != 0, ib&IB_Z != 0, ib&IB_S != 0,
		ib&IB_D != 0, ib&IB_W != 0, ib&IB_M != 0, ib&IB_H != 0, ib&IB_T != 0)
}
...
func (__ *CommandBuffer) LastChangeTime() int32 {
	return Min(__.LastDirectionTime(), Abs(__.ab), Abs(__.bb), Abs(__.cb),
		Abs(__.xb), Abs(__.yb), Abs(__.zb), Abs(__.sb), Abs(__.db), Abs(__.wb), Abs(__.hb), Abs(__.tb),
		Abs(__.mb))
}
...
type AiInput struct {
	dir, dirt, at, bt, ct, xt, yt, zt, st, dt, wt, ht, tt, mt int32
}
...
func (ai *AiInput) Update(level float32) {
	if sys.intro != 0 {
		...
		ai.dt, ai.wt, ai.ht, ai.tt = 0, 0, 0, 0
		ai.mt = 0
		return
	}
...
func (ai *AiInput) h() bool {
	return ai.ht != 0
}
func (ai *AiInput) t() bool {
	return ai.tt != 0
}
...
func ReadCommand(name, cmdstr string, kr *CommandKeyRemap) (*Command, error) {
	...
	for _, cestr := range cmd {
		...
		for len(cestr) > 0 {
			...
			case 'h':
				if tilde {
					ce.key = append(ce.key, kr.nh)
				} else {
					ce.key = append(ce.key, kr.h)
				}
				tilde = false
			case 't':
				if tilde {
					ce.key = append(ce.key, kr.nt)
				} else {
					ce.key = append(ce.key, kr.t)
				}
				tilde = false
			case 'm':
				if tilde {
...
func (cl *CommandList) Input(i int, facing int32, aiLevel float32, ib InputBits) bool {
	...
	if _else {
		var L, R, U, D, a, b, c, x, y, z, s, d, w, h, t, m bool
		if i < 0 {
			i = ^i
			if i < len(sys.aiInput) {
				...
				h = sys.aiInput[i].h() || ib&IB_H != 0
				t = sys.aiInput[i].t() || ib&IB_T != 0
			...
			if in < len(sys.keyConfig) {
				...
				if joy == -1 {
					...
					h = sys.keyConfig[in].h() || ib&IB_H != 0
					t = sys.keyConfig[in].t() || ib&IB_T != 0
			...
			if in < len(sys.joystickConfig) {
				...
				if joyS >= 0 {
					...
					if !h {
						h = sys.joystickConfig[in].h()
					}
					if !t {
						t = sys.joystickConfig[in].t()
					}
					if !m {
		...
		cl.Buffer.Input(B, D, F, U, a, b, c, x, y, z, s, d, w, h, t, m)
	}
	return step
}
